...
data = data[['observation_start_date', 'lat', 'lon','income_diff', 'area_id']]
data = data[data['income_diff']>0]
...
with row1_1:
    ...
    month_selected = st.slider("Choose month", 1, 12)
...
with row2_1:
    ...
    map(data, miami[0],miami[1], zoom_level)
...
sub_selection = filtered[['area_id', 'income_diff']]
...
with row2_2:
    st.write("Top 10 performing areas accumulated over the period")

    #chart_data = pd.DataFrame(sub_selection_v2[:10], columns=['income_diff'])
    #st.area_chart(chart_data, height=520)
    st.write(sub_selection_v2.head(10))

with row2_3:
    st.write("Bottom 10 performing areas accumulated over the period")
